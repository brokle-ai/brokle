...
import { useTestEvaluator, type TestOptions } from '../hooks/use-test-evaluator'
import type { Evaluator, TestSummary, TestExecution, EvaluatorPreview } from '../types'
...
/**
 * Dialog for testing evaluators before activation.
 *
 * Features:
...
export function TestEvaluatorDialog({
  ...
  const { result, startTest, resetTest, isRunning } = useTestEvaluator(
    projectId,
    evaluator.id,
    {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Run a test evaluation against sample spans to verify the evaluator works correctly before activation.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-6 py-4 pr-4">
              ...
              {result.status === 'completed' && result.summary && (
                <TestResults
                  ...
                  evaluatorPreview={result.evaluatorPreview}
                />
              )}
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-muted/50 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">Evaluator Configuration</span>
          <Badge variant="outline" className="text-xs">
            {evaluator.scorer_type.toUpperCase()}
          </Badge>
        </div>
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className="ml-2 font-medium">
              {Math.round(evaluator.sampling_rate * 100)}%
            </span>
          </div>
          <div>
            <span className="text-muted-foreground">Target:</span>
            <span className="ml-2 font-medium">{evaluator.target_scope}</span>
          </div>
          {evaluator.span_names && evaluator.span_names.length > 0 && (
            <div className="col-span-2">
              <span className="text-muted-foreground">Span Names:</span>
              <span className="ml-2 font-medium">
                {evaluator.span_names.join(', ')}
              </span>
            </div>
          )}
        </div>
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800">
          ...
          <div>
            ...
            <p className="text-sm text-green-600 dark:text-green-400">
              Your evaluator is ready to be activated.
            </p>
          </div>
        </div>
      ) : (
        <div className="flex items-center gap-3 p-4 rounded-lg bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-800">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-600 dark:text-yellow-400">
              No spans were successfully scored. Check your evaluator configuration.
            </p>
          </div>
        </div>
      )}
      ...
      {evaluatorPreview && (
        <div className="rounded-lg border p-4 space-y-2">
          <p className="text-sm font-medium">Evaluator Configuration</p>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-muted-foreground">Scorer Type:</span>
              <span className="ml-2 font-medium">{evaluatorPreview.scorer_type}</span>
            </div>
            {evaluatorPreview.matching_count !== undefined && (
              <div>
                <span className="text-muted-foreground">Est. Matching:</span>
                <span className="ml-2 font-medium">
                  {evaluatorPreview.matching_count} spans
                </span>
              </div>
            )}
          </div>
          {evaluatorPreview.filter_description && (
