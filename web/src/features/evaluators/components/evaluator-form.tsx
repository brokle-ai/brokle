...
// Default output schema for new evaluators
...
function extractScorerDefaults(evaluator?: Evaluator) {
  if (!evaluator) {
    return {
      llm_credential_id: '',
      llm_model: '',
  ...
  if (evaluator.scorer_type === 'llm') {
    const config = evaluator.scorer_config as LLMScorerConfig
  ...
  if (evaluator.scorer_type === 'builtin') {
    const config = evaluator.scorer_config as BuiltinScorerConfig
    return {
      llm_credential_id: '',
      llm_model: '',
      llm_system_prompt: '',
  ...
  if (evaluator.scorer_type === 'regex') {
    const config = evaluator.scorer_config as RegexScorerConfig
    return {
      llm_credential_id: '',
      llm_model: '',
      llm_system_prompt: '',
...
type EvaluatorFormData = z.infer<typeof evaluatorFormSchema>

interface EvaluatorFormProps {
  evaluator?: Evaluator
  onSubmit: (data: CreateEvaluatorRequest) => void
  onCancel: () => void
  isLoading?: boolean
  orgId?: string
}
...
export function EvaluatorForm({
  ...
  const scorerDefaults = extractScorerDefaults(evaluator)
  ...
  const form = useForm<EvaluatorFormData>({
    ...
    defaultValues: {
      ...
      target_scope: evaluator?.target_scope ?? 'span',
      span_names: evaluator?.span_names?.join(', ') ?? '',
      sampling_rate: evaluator?.sampling_rate ?? 1,
      filter: (evaluator?.filter ?? []) as FilterClause[],
      variable_mapping: (evaluator?.variable_mapping ?? []) as VariableMap[],
      scorer_type: evaluator?.scorer_type ?? 'llm',
      ...scorerDefaults,
    },
  })
  ...
  const handleFormSubmit = (data: EvaluatorFormData) => {
    let scorer_config: CreateEvaluatorRequest['scorer_config']
      ...
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Describe what this evaluator evaluates"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Active evaluators will automatically score matching spans.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <div className="flex justify-end gap-2 pt-4 border-t">
          ...
          <Button type="submit" disabled={isLoading}>
            {isLoading ? 'Saving...' : evaluator ? 'Update Evaluator' : 'Create Evaluator'}
          </Button>
        </div>
      </form>
    </Form>
