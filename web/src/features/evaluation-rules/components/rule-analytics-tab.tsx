...
import { useRuleAnalyticsQuery } from '../hooks/use-rule-analytics'
import type { RuleAnalyticsParams, TimeSeriesPoint, DistributionBucket, ErrorSummary } from '../types'
...
export function RuleAnalyticsTab({ projectId, ruleId }: RuleAnalyticsTabProps) {
  const [period, setPeriod] = useState<Period>('7d')

  const params: RuleAnalyticsParams = useMemo(() => ({
    period,
  }), [period])
  ...
  const {
    ...
    isLoading,
    error,
  } = useRuleAnalyticsQuery(projectId, ruleId, params)
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Analytics will appear here once the rule starts scoring spans
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      {/* Period Selector */}
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium">Rule Analytics</h3>
