...
import type { RuleExecution, TriggerType } from '../types'
...
export function RuleExecutionsTable({
  ...
  const { data, isLoading, isError, error } = useRuleExecutionsQuery(
    projectId,
    ruleId,
    {
      page,
  ...
  const { data: refreshedData } = useRuleExecutionsQuery(projectId, ruleId, {
    page,
    limit,
    triggerType: triggerFilter === 'all' ? undefined : triggerFilter,
