...
import type { EvaluationRule, ScorerType, RuleStatus } from '../types'
import type { Pagination } from '@/lib/api/core/types'
import { RulesToolbar } from './rules-toolbar'
import type { RuleSortField } from '../hooks/use-rules-table-state'
...
interface RulesTableProps {
  ...
  onStatusChange: (status: RuleStatus | null) => void
  onPageChange: (page: number, pageSize?: number) => void
  onSortChange: (sortBy: RuleSortField | null, sortOrder: 'asc' | 'desc' | null) => void
  onReset: () => void
  hasActiveFilters: boolean

  // Actions
  onStatusToggle?: (ruleId: string, newStatus: RuleStatus) => void
  onEdit?: (rule: EvaluationRule) => void
  onDuplicate?: (rule: EvaluationRule) => void
  onViewLogs?: (rule: EvaluationRule) => void
  onDelete?: (rule: EvaluationRule) => void
}

/**
 * Evaluation Rules Table Component
 *
 * Features:
...
export function RulesTable({
  ...
  const columns = useMemo<ColumnDef<EvaluationRule>[]>(
    () => [
      {
        accessorKey: 'name',
        header: () => (
        ...
        cell: ({ row }) => (
          <Link
            href={`/projects/${projectSlug}/evaluation-rules/${row.original.id}`}
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          <p className="text-muted-foreground">Failed to load evaluation rules</p>
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center">
                  <div className="flex flex-col items-center gap-2">
                    <p className="text-muted-foreground">No evaluation rules found</p>
                    {hasActiveFilters && (
...
function SortableHeader({
  ...
  label: string
  field: RuleSortField
  currentSort: RuleSortField
  currentOrder: 'asc' | 'desc'
  onSort: (field: RuleSortField | null, order: 'asc' | 'desc' | null) => void
}) {
...
function StatusToggle({
  ...
  rule: EvaluationRule
  onToggle?: (ruleId: string, newStatus: RuleStatus) => void
}) {
  const isActive = rule.status === 'active'

  const handleToggle = () => {
    if (onToggle) {
      onToggle(rule.id, isActive ? 'inactive' : 'active')
    }
  }
  ...
  return (
    <div className="flex items-center gap-2">
      <Switch
        ...
        disabled={!onToggle || rule.status === 'paused'}
        aria-label={`Toggle rule ${rule.name} ${isActive ? 'off' : 'on'}`}
      />
...
function getStatusConfig(status: RuleStatus) {
  switch (status) {
    case 'active':
      return {
        label: 'Active',
