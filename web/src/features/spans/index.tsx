...
import { SpansTable } from './components/spans-table'
...
export function Spans({ projectSlug }: SpansProps) {
  ...
  return (
    <div className='-mx-4 flex flex-1 flex-col overflow-auto px-4 py-1'>
      ...
      {/* Data table */}
      {!error && hasProject && !isInitialLoad && !isEmptyProject && (
        <SpansTable data={data} totalCount={totalCount} isFetching={isFetching} />
      )}
    </div>
  )
}
...
// Hooks
export { useProjectSpans, useSpansTableState } from './hooks/use-project-spans'

// Types
export type { UseProjectSpansReturn, UseSpansTableStateReturn } from './hooks/use-project-spans'
