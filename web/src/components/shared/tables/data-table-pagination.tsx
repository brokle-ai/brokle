    ...
    >
      {showSelectedRows && (
        <div className='text-muted-foreground hidden flex-1 text-sm sm:block'>
          ...
          {serverPagination ? serverPagination.total : table.getFilteredRowModel().rows.length} row(s) selected.
        </div>
      )}
      <div className='flex items-center sm:space-x-6 lg:space-x-8'>
        ...
        <div className='flex w-[100px] items-center justify-center text-sm font-medium'>
          Page {serverPagination ? serverPagination.page : table.getState().pagination.pageIndex + 1} of{' '}
        ...
        <div className='flex items-center space-x-2'>
          ...
          <Button
            variant='outline'
            className='hidden h-8 w-8 p-0 lg:flex'
            onClick={() => table.setPageIndex((serverPagination?.totalPages ?? table.getPageCount()) - 1)}
            disabled={serverPagination ? !serverPagination.hasNextPage : !table.getCanNextPage()}
          >
