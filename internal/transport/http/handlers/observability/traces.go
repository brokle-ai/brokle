...
// CreateTrace handles POST /api/v1/observability/traces
...
// @Produce json
// @Security BearerAuth
// @Param request body CreateTraceRequest true "Trace creation data"
// @Success 201 {object} response.SuccessResponse{data=TraceResponse} "Trace created successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid request payload or trace data"
// @Failure 409 {object} response.ErrorResponse "Trace with external_trace_id already exists"
// @Failure 422 {object} response.ErrorResponse "Validation failed"
// @Failure 500 {object} response.ErrorResponse "Internal server error"
// @Router /api/v1/observability/traces [post]
func (h *Handler) CreateTrace(c *gin.Context) {
	...
	trace, err := h.requestToTrace(&req)
	if err != nil {
		response.BadRequest(c, "Invalid trace data", err.Error())
		return
	}

	// Create trace via service
	createdTrace, err := h.services.GetTraceService().CreateTrace(c.Request.Context(), trace)
	if err != nil {
		...
		if observability.IsConflictError(err) {
			response.Conflict(c, "Trace already exists")
			return
		}
		response.InternalServerError(c, "Failed to create trace")
		return
	}

	// Convert to response
	resp := h.traceToResponse(createdTrace)
...
// UpdateTrace handles PUT /api/v1/observability/traces/{id}
...
// @Security BearerAuth
// @Param id path string true "Trace ID (ULID format)"
// @Param request body UpdateTraceRequest true "Trace update data"
// @Success 200 {object} response.SuccessResponse{data=TraceResponse} "Trace updated successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid trace ID or request payload"
// @Failure 404 {object} response.ErrorResponse "Trace not found"
// @Failure 422 {object} response.ErrorResponse "Validation failed"
// @Failure 500 {object} response.ErrorResponse "Internal server error"
// @Router /api/v1/observability/traces/{id} [put]
func (h *Handler) UpdateTrace(c *gin.Context) {
	...
	// Convert request to domain entity
	trace, err := h.updateRequestToTrace(&req, traceID)
	if err != nil {
		response.BadRequest(c, "Invalid trace data", err.Error())
		return
	}

	// Update trace via service
	updatedTrace, err := h.services.GetTraceService().UpdateTrace(c.Request.Context(), trace)
	if err != nil {
		if observability.IsNotFoundError(err) {
			response.NotFound(c, "Trace")
			return
		...
		response.InternalServerError(c, "Failed to update trace")
		return
	}

	// Convert to response
	resp := h.traceToResponse(updatedTrace)
	response.Success(c, resp)
}

// DeleteTrace handles DELETE /api/v1/observability/traces/{id}
...
// @Security BearerAuth
// @Param id path string true "Trace ID (ULID format)"
// @Success 200 {object} response.MessageResponse "Trace deleted successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid trace ID format"
// @Failure 404 {object} response.ErrorResponse "Trace not found"
// @Failure 500 {object} response.ErrorResponse "Internal server error"
// @Router /api/v1/observability/traces/{id} [delete]
...
// CreateTracesBatch handles POST /api/v1/observability/traces/batch
...
// @Produce json
// @Security BearerAuth
// @Param request body BatchCreateTracesRequest true "Batch trace creation data"
// @Success 201 {object} response.SuccessResponse{data=BatchCreateTracesResponse} "Traces created successfully"
// @Failure 400 {object} response.ErrorResponse "Invalid request payload or trace data"
// @Failure 422 {object} response.ErrorResponse "Validation failed"
// @Failure 500 {object} response.ErrorResponse "Internal server error"
// @Router /api/v1/observability/traces/batch [post]
func (h *Handler) CreateTracesBatch(c *gin.Context) {
	var req BatchCreateTracesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request body", err.Error())
		return
	}
	...
	for _, traceReq := range req.Traces {
		trace, err := h.requestToTrace(&traceReq)
		if err != nil {
			response.BadRequest(c, "Invalid trace data", err.Error())
			return
		}
	...
	// Create traces batch via service
	createdTraces, err := h.services.GetTraceService().CreateTracesBatch(c.Request.Context(), traces)
	if err != nil {
		if observability.IsValidationError(err) {
			response.ValidationError(c, "Validation failed", err.Error())
			return
		}
		response.InternalServerError(c, "Failed to create traces batch")
		return
	}

	// Convert to response
	var traceResponses []TraceResponse
	for _, trace := range createdTraces {
		traceResponses = append(traceResponses, h.traceToResponse(trace))
	}

	resp := BatchCreateTracesResponse{
		Traces:         traceResponses,
...
// requestToTrace converts a CreateTraceRequest to a Trace domain entity
func (h *Handler) requestToTrace(req *CreateTraceRequest) (*observability.Trace, error) {
	trace := &observability.Trace{
		ExternalTraceID: req.ExternalTraceID,
		Name:            req.Name,
		Tags:            req.Tags,
	...
	// Parse parent_trace_id
	if req.ParentTraceID != "" {
		parentTraceID, err := ulid.Parse(req.ParentTraceID)
		if err != nil {
			return nil, err
		}
...
// updateRequestToTrace converts an UpdateTraceRequest to a Trace domain entity
func (h *Handler) updateRequestToTrace(req *UpdateTraceRequest, traceID ulid.ULID) (*observability.Trace, error) {
	trace := &observability.Trace{
		ID:   traceID,
		Name: req.Name,
	}
