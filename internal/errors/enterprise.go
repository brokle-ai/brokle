...
func buildUsageActions(currentTier, limitType string) []ActionSuggestion {
	actions := []ActionSuggestion{
		...
		},
		{
			...
			URL:         "https://docs.brokle.ai/usage-limits",
			UTMSource:   "api",
...
func buildEnterpriseActions(currentTier string) []ActionSuggestion {
	return []ActionSuggestion{
		...
		},
		{
			...
			URL:         "https://brokle.ai/pricing?tier=enterprise&utm_source=api&utm_campaign=enterprise_access&utm_content=license_required",
			UTMSource:   "api",
			UTMCampaign: "enterprise_access",
			UTMContent:  "license_required",
			Primary:     false,
...
func buildSupportInfo() SupportInfo {
	return SupportInfo{
		...
		ChatURL:      "https://brokle.ai/chat",
		DocsURL:      "https://docs.brokle.ai",
		CommunityURL: "https://community.brokle.ai",
	}
}
...
func buildUpgradeURL(tier, feature string) string {
	return fmt.Sprintf("https://brokle.ai/pricing?tier=%s&utm_source=api&utm_campaign=feature_upgrade&utm_content=%s", tier, feature)
}

func buildTrialURL(feature string) string {
	return fmt.Sprintf("https://brokle.ai/trial?utm_source=api&utm_campaign=feature_trial&utm_content=%s", feature)
}

func buildSalesURL(feature string) string {
	return fmt.Sprintf("https://brokle.ai/contact?utm_source=api&utm_campaign=enterprise_inquiry&utm_content=%s", feature)
}

func buildUsageUpgradeURL(limitType string) string {
	return fmt.Sprintf("https://brokle.ai/pricing?utm_source=api&utm_campaign=usage_upgrade&utm_content=%s", limitType)
}

func buildFeatureDocsURL(feature string) string {
	return fmt.Sprintf("https://docs.brokle.ai/features/%s?utm_source=api&utm_campaign=feature_docs&utm_content=%s", feature, feature)
}
