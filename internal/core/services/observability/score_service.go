...
func (s *ScoreService) CreateScore(ctx context.Context, score *observability.Score) error {
	if score.ProjectID == uuid.Nil {
		return appErrors.InvalidParam("project_id", "is required", appErrors.WithDetails("score must have a valid project_id"))
	}
	if score.Name == "" {
		return appErrors.InvalidParam("name", "is required", appErrors.WithDetails("score name cannot be empty"))
	}
	...
	if !hasTraceLinkage && !hasExperimentLinkage {
		return appErrors.BadRequest(
			...
			appErrors.WithDetails("scores must be linked to a trace/span or experiment"),
		)
	}
	...
	if err := s.scoreRepo.Create(ctx, score); err != nil {
		return appErrors.Internal("failed to create score", err)
	}
...
func (s *ScoreService) UpdateScore(ctx context.Context, score *observability.Score) error {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return appErrors.NotFound("score", appErrors.WithMessage("score "+score.ID.String()+" not found"))
		}
		return appErrors.Internal("failed to get score", err)
	}
	...
	if err := s.scoreRepo.Update(ctx, existing); err != nil {
		return appErrors.Internal("failed to update score", err)
	}
...
func (s *ScoreService) DeleteScore(ctx context.Context, id uuid.UUID) error {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return appErrors.NotFound("score", appErrors.WithMessage("score "+id.String()+" not found"))
		}
		return appErrors.Internal("failed to get score", err)
	}
	if err := s.scoreRepo.Delete(ctx, id); err != nil {
		return appErrors.Internal("failed to delete score", err)
	}
...
func (s *ScoreService) GetScoreByID(ctx context.Context, id uuid.UUID) (*observability.Score, error) {
	...
	if err != nil {
		return nil, appErrors.NotFound("score", appErrors.WithMessage("score "+id.String()+" not found"))
	}
...
func (s *ScoreService) GetScoresByTraceID(ctx context.Context, traceID string) ([]*observability.Score, error) {
	if traceID == "" {
		return nil, appErrors.InvalidParam("trace_id", "is required", appErrors.WithDetails("scores query requires a valid trace_id"))
	}

	scores, err := s.scoreRepo.GetByTraceID(ctx, traceID)
	if err != nil {
		return nil, appErrors.Internal("failed to get scores", err)
	}
...
func (s *ScoreService) GetScoresBySpanID(ctx context.Context, spanID string) ([]*observability.Score, error) {
	if spanID == "" {
		return nil, appErrors.InvalidParam("span_id", "is required", appErrors.WithDetails("scores query requires a valid span_id"))
	}
...
func (s *ScoreService) GetScoresByFilter(ctx context.Context, filter *observability.ScoreFilter) ([]*observability.Score, error) {
	...
	if err != nil {
		return nil, appErrors.Internal("failed to get scores by filter", err)
	}
...
func (s *ScoreService) CreateScoreBatch(ctx context.Context, scores []*observability.Score) error {
	if len(scores) == 0 {
		return appErrors.InvalidParam("scores", "array cannot be empty", appErrors.WithDetails("batch create requires at least one score"))
	}

	// Validate all scores
	for i, score := range scores {
		if score.ProjectID == uuid.Nil {
			return appErrors.InvalidParam(fmt.Sprintf("scores[%d].project_id", i), "is required", appErrors.WithDetails("all scores must have valid project_id"))
		}
		if score.Name == "" {
			return appErrors.InvalidParam(fmt.Sprintf("scores[%d].name", i), "is required", appErrors.WithDetails("all scores must have a name"))
		}
		...
		if !hasTraceLinkage && !hasExperimentLinkage {
			return appErrors.BadRequest(
				fmt.Sprintf("scores[%d]: must have either trace_id+span_id or experiment_id", i),
				appErrors.WithDetails("each score in batch must be linked to a trace/span or experiment"),
			)
		}
	...
	if err := s.scoreRepo.CreateBatch(ctx, scores); err != nil {
		return appErrors.Internal("failed to create score batch", err)
	}
...
func (s *ScoreService) CountScores(ctx context.Context, filter *observability.ScoreFilter) (int64, error) {
	...
	if err != nil {
		return 0, appErrors.Internal("failed to count scores", err)
	}
...
func (s *ScoreService) validateScoreData(score *observability.Score) error {
	...
	case observability.ScoreTypeNumeric:
		if score.Value == nil {
			return appErrors.InvalidParam("value", "is required for NUMERIC type")
		}
		if score.StringValue != nil {
			return appErrors.InvalidParam("string_value", "not allowed for NUMERIC type")
		}

	case observability.ScoreTypeCategorical:
		if score.StringValue == nil {
			return appErrors.InvalidParam("string_value", "is required for CATEGORICAL type")
		}
		if score.Value != nil {
			return appErrors.InvalidParam("value", "not allowed for CATEGORICAL type")
		}

	case observability.ScoreTypeBoolean:
		if score.Value == nil {
			return appErrors.InvalidParam("value", "is required for BOOLEAN type")
		}
		if *score.Value != 0 && *score.Value != 1 {
			return appErrors.InvalidParam("value", "must be 0 (false) or 1 (true)")
		}
		if score.StringValue != nil {
			return appErrors.InvalidParam("string_value", "not allowed for BOOLEAN type")
		}

	default:
		return appErrors.InvalidParam("type", "must be NUMERIC, CATEGORICAL, or BOOLEAN")
	}
...
func (s *ScoreService) validateScoreTargets(ctx context.Context, score *observability.Score) error {
	if score.TraceID != nil && *score.TraceID != "" {
		...
		if err != nil {
			return appErrors.NotFound("trace", appErrors.WithMessage("trace "+*score.TraceID+" not found"))
		}
	}
	...
	if score.SpanID != nil && *score.SpanID != "" {
		...
		if err != nil {
			return appErrors.NotFound("span", appErrors.WithMessage("span "+*score.SpanID+" not found"))
		}
	}
