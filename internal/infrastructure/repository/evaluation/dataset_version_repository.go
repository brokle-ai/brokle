...
func (r *datasetVersionRepository) GetByVersionNumber(ctx context.Context, datasetID uuid.UUID, versionNum int) (*evalDomain.DatasetVersion, error) {
	...
	if err != nil {
		if db.IsNoRows(err) {
			return nil, appErrors.NotFound("dataset_version", appErrors.WithOp("repo.dataset_version.get_by_number"))
		}
		return nil, appErrors.Internal("get dataset version", err, appErrors.WithOp("repo.dataset_version.get_by_number"))
	}
	return datasetVersionFromRow(&row)
}
