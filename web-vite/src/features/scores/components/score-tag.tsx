...
export function ScoreTag({
  ...
  const tagContent = (
    ...
    >
      ...
      <span className={cn('text-xs font-semibold', textClass)}>{formattedValue}</span>
  ...
  return (
    <HoverCard openDelay={300}>
      <HoverCardTrigger asChild>{tagContent}</HoverCardTrigger>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <span
          className={cn('inline-flex items-center px-2 py-0.5 text-xs font-medium rounded', sourceClassName)}
        >
          {sourceLabel}
        </span>
      </div>
...
export function ScoreTagList({
  ...
  if (scores.length === 0) {
    return <span className="text-sm text-muted-foreground">No scores</span>
  }

  return (
    <div className={cn('flex flex-wrap items-center gap-1.5', className)}>
      {visibleScores.map((score) => (
        <ScoreTag key={score.id} score={score} onDelete={onDelete} compact />
      ))}
      {remainingCount > 0 && (
