...
export function ScoreAnalyticsDashboard({
  ...
  const { data: scoreNames, isLoading: isLoadingNames } =
    useScoreNamesQuery(projectId)
  ...
  const clearCompareScore = () => setCompareScore('')

  if (!isLoadingNames && (!scoreNames || scoreNames.length === 0)) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <p className="mb-2 text-muted-foreground">
          No scores found in this project
        </p>
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            <SelectTrigger
              id="score-select"
              className="w-[200px]"
              aria-label="Select primary score"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger
                id="compare-select"
                className="w-[200px]"
                aria-label="Select comparison score"
              >
      ...
      {error && (
        <div className="flex flex-col items-center justify-center rounded-lg border border-destructive/20 bg-destructive/5 py-16 text-center">
          <p className="mb-2 text-destructive">Failed to load analytics</p>
          <p className="text-sm text-muted-foreground">
