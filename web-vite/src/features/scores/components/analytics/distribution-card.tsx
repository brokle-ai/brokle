...
import type { DistributionBin } from '../../api/types'
...
function formatBinLabel(binStart: number, binEnd: number): string {
  if (binStart === binEnd) return binStart.toFixed(2)
        ...
        >
          ...
          <Tooltip
            content={({ active, payload }) => {
              ...
              const datum = payload[0].payload as ChartDataPoint
              return (
                <div className="rounded-lg border bg-popover p-3 shadow-lg">
                  <p className="mb-1 text-sm font-medium">{datum.label}</p>
          ...
          />
          <Bar dataKey="value" name={scoreName} radius={[4, 4, 0, 0]}>
            {data.map((_, index) => (
