import type { ExperimentScoreDiff, ScoreComparisonRow } from '../api/types'
  ...
  for (const row of scoreRows) {
    ...
    for (const [expId, data] of Object.entries(row.experiments)) {
      ...
      if (data.diff.type === 'CATEGORICAL') {
        // Categorical comparisons count as unchanged (we don't
        // classify categorical as improved/regressed).
        result.unchanged++
        continue
      }
