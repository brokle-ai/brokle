...
const WIDGET_TYPES: Array<{
  ...
  },
  {
    value: 'heatmap',
    label: 'Heatmap',
    description: 'Value density visualization',
  },
  {
    value: 'histogram',
    label: 'Histogram',
...
export function WidgetForm({
  ...
  const isValid =
    title.trim().length > 0 && (isTextWidget || query.measures.length > 0)
      ...
      {!isTextWidget ? (
        ...
        <div className="rounded-lg border bg-muted/50 p-4">
          <p className="text-sm text-muted-foreground">
            Text widgets display static content. Configure the content after
            creating the widget.
          </p>
        </div>
      )}
