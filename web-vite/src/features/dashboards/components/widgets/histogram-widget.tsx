...
export function HistogramWidget({
  ...
  const stats = useMemo((): HistogramStats | null => {
    ...
    const weightedSum = valuesWithCounts.reduce(
      (sum, v) => sum + v.value * v.count,
      0,
    )
              ...
              {stats && (
                <ReferenceLine
                  x={
                    chartData.find((d) => {
                      if (
                        d.lower_bound !== undefined &&
                        d.upper_bound !== undefined
                      ) {
                        return (
                          stats.p95 >= d.lower_bound && stats.p95 < d.upper_bound
                        )
                      }
                      const nextBucket = chartData[chartData.indexOf(d) + 1]
                      if (nextBucket) {
                        return (
                          stats.p95 >= d.midpoint &&
                          stats.p95 < nextBucket.midpoint
                        )
                      }
                      return false
                    })?.bucketLabel
