...
export function SharedVariablesPanel({ className }: SharedVariablesPanelProps) {
  ...
  useEffect(() => {
    if (useSharedVariables && allVariables.length > 0) {
      const needsInit = allVariables.some(
        (v) => sharedVariables[v] === undefined,
      )
      if (needsInit) {
        initializeVariables()
      }
  ...
  return (
    <Collapsible
      defaultOpen={useSharedVariables}
          ...
          <CardContent className="pt-0">
            {!useSharedVariables ? (
              <p className="text-sm text-muted-foreground">
                Enable shared variables to use the same variable values across
                all windows. Variables are detected automatically from message
                templates.
              </p>
              ...
              <p className="text-sm text-muted-foreground italic">
                ...
                syntax in your messages to add variables.
              </p>
            ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {allVariables.map((variable) => {
                    const usageCount = windows.filter((w) =>
                      extractVariablesFromMessages(w.messages).includes(
                        variable,
                      ),
                    ).length
