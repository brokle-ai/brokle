...
export function SaveAsPromptDialog({
  ...
  const [saveMode, setSaveMode] = useState<SaveMode>(
    loadedFromPromptId ? 'version' : 'new',
  )
  ...
  // Backend's chat prompt wire format uses `type: 'message'` + role + content.
  ...
  const createVersionMutation = useMutation({
    mutationFn: async () => {
      ...
      return createPromptVersion(projectId, loadedFromPromptId, {
        template,
  ...
  const isLoading =
    createPromptMutation.isPending || createVersionMutation.isPending
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Save your current messages as a prompt for reuse and version
            control.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          {!hasValidMessages && (
            <div className="text-sm text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/50 p-3 rounded-md">
              No messages with content to save. Add content to at least one
