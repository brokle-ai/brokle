...
function analyzeColumns(headers: string[], rows: string[][]): ParsedColumn[] {
  return headers.map((name, colIndex) => {
    // `noUncheckedIndexedAccess` makes row[colIndex] `string | undefined`.
    // Missing cells collapse to '' so downstream filtering treats them
    // as empty/null.
    const values = rows.map(row => row[colIndex] ?? '')
    const nonEmptyValues = values.filter((v): v is string => v !== '')
