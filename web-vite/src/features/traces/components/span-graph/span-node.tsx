...
function formatDurationLabel(durationNs: number | undefined): string | undefined {
...
function SpanNodeComponent({ data, selected }: NodeProps<SpanNodeData>) {
  ...
  const duration = formatDurationLabel(data.span.duration)
        ...
        <TooltipTrigger asChild>
          <div
            className={cn(
              ...
              data.hasError &&
                'border-red-500 dark:border-red-400 bg-red-50 dark:bg-red-950/40',
        ...
        <TooltipContent side="top" className="max-w-[300px]">
          <div className="space-y-1">
            ...
            {data.statusCode && (
              <p className="text-xs">
                Status:{' '}
                <span className={data.hasError ? 'text-red-500' : 'text-green-500'}>
                  {data.statusCode}
                </span>
              </p>
            )}
