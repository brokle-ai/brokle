...
import { tracesKeys, updateTraceTags } from '../api/queries'
 ...
 *
 * Backend normalises on write (lowercase, unique, sorted) and enforces
 * max 50 tags per trace / max 100 chars per tag — we mirror those
 * constraints in the UI so the request is never rejected by validation.
 */
export function TraceTagsEditor({
  ...
  const handleRemoveTag = (tagToRemove: string) => {
    mutation.mutate(tags.filter((t) => t !== tagToRemove))
  }
