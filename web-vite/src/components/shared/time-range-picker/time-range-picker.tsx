...
export function TimeRangePicker({
  ...
  const [customFrom, setCustomFrom] = useState<Date | undefined>(() =>
    value.relative === 'custom' && value.from ? new Date(value.from) : undefined,
  )
  const [customTo, setCustomTo] = useState<Date | undefined>(() =>
    value.relative === 'custom' && value.to ? new Date(value.to) : undefined,
  )
  ...
  const handleRelativeSelect = (option: RelativeOption) => {
    onChange({ relative: option.value, from: undefined, to: undefined })
    setOpen(false)
    setShowCustom(false)
  }
